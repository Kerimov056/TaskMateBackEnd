...
public class WorkspacesController : ControllerBase
{
    ...
    [HttpGet("[action]")]
    public async Task<IActionResult> GetAllIsArchıve(string AppUserId)
    {
        var workspaces = await _workspaceService.GetArchiveAsync(AppUserId);
        return Ok(workspaces);
    }
    ...
    [HttpPut("[action]")]
    public async Task<IActionResult> UpdateIsArchive(string AppUserId, Guid WorkspaceId)
    {
        await _workspaceService.UpdateArchive(AppUserId, WorkspaceId);
        return Ok();
    }
    ...
    [HttpGet("[action]")]
    public async Task<IActionResult> GetGlobalSearch(string searchTerm)
    {
        var globalSerach =await _workspaceService.GetGlobalSearch(searchTerm);
        return Ok(globalSerach);
    }
}
